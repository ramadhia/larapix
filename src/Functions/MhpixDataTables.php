...
trait MhpixDataTables{

	public static function data_output ( $columns, $d )
	{
		$data	= (array)$d;
		$out	= array();
		return $data;exit();
	...
	public static function simple ( $request, $table, $primaryKey, $columns ){
		...
		// $data = self::sql_exec( $db, $bindings,
			...
			//  $limit"
		// );
		// $data	= DB::raw("SELECT `".implode("`, `", self::pluck($columns, 'db') )."` FROM `$table` $where $order $limit");
		$getData	= DB::table($table)->selectRaw('`'.implode("`, `", self::pluck($columns, 'db')).'`');
					if($where){
						$getData->whereRaw($where);
					}
					// ->orderbyRaw($order)
		$data		= $getData->get();
		...
		// 	 $where"
		// );
		$getFilter			= DB::table($table);
		if($where){
			$getFilter->whereRaw($where);
		}
		$resFilterLength	= (array)$getFilter->get();
		...
		$getTotal			= DB::table($table);
		if($where){
			$getTotal->whereRaw($where);
		}
		$resTotalLength	= (array)$getFilter->count();
		$recordsTotal	= $resFilterLength;
		/*
		 * Output
		 */
		// return $request;
